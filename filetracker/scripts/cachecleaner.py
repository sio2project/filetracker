...
class CacheCleaner(object):
    ...
    FileIndexEntry = collections.namedtuple('FileIndexEntry', ['file_info', 'client'])
    ...
    def _scan_disk(self):
        ...
        self.clients = [Client(cache_dir=dir, remote_store=None) for dir in dirs]
        self.file_index = []
        for client in self.clients:
            for file_info in client.list_local_files():
                self.file_index.append(
                    CacheCleaner.FileIndexEntry(file_info=file_info, client=client)
                )

    def _analyze_file_index(self):
        logger.info("Analyzing cache...")

        self.file_index.sort(key=lambda x: (-x.file_info.mtime, x.file_info.size))
        cache_size = 0
        delete_from_index = None
        do_cleaning = False
        for i, entry in enumerate(self.file_index):
            cache_size += entry.file_info.size
            if delete_from_index is None and cache_size >= self.cleaning_level:
                delete_from_index = i
            if cache_size >= self.cache_size_limit:
                do_cleaning = True

        logger.info("Analysis done. Cache size: %s.", format_size_with_unit(cache_size))
        if not do_cleaning:
    ...
    def _clean_cache(self, delete_from_index):
        ...
        deleted_files_cnt = 0
        deleted_bytes = 0
        assert self.file_index[0].file_info.mtime >= self.file_index[-1].file_info.mtime
        for entry in self.file_index[delete_from_index:]:
            logger.debug(
                "Deleting file: %s from store located at: %s",
                entry.file_info.name,
                entry.client.local_store.dir,
            )
            try:
                ...
                logger.warning(
                    'During deleting file %s occurred following ' 'exception: %s',
                    entry.file_info.name,
                    e,
                    exc_info=True,
                )
        ...
        logger.info(
            "Cleaning done. Deleted %d files, total %s.",
            deleted_files_cnt,
            format_size_with_unit(deleted_bytes),
        )
...
def main():
    ...
    if not args.cache_size_limit:
        parser.error(
            "Missing --cache-size-limit option. " "Try --help for more details."
        )
    ...
    if args.quiet:
        level = None
    logging.basicConfig(
        format="%(asctime)-23s %(name)s %(levelname)s: %(message)s", level=level
    )
...
def parse_units(text, units):
    ...
    for ch in list(str(text).strip()):
        if ch.isdigit():
            ...
            unit = units[ch]
            if not value:
                raise ValueError("Unit without numeric value: {}".format(text))
...
def format_with_unit(number, size_units):
    ...
    for unit, size in units:
        if number >= size:
            return "{amount:.3f}{unit}".format(amount=float(number) / size, unit=unit)
